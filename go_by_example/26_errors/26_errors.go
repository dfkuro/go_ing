...
* overloaded single result / error value sometimes used in C. Go’s approach makes it
* easy to see which functions return errors and to handle them using the same language
...
func makeTea(arg int) error {
	if arg == 2 {
		...
		// way to do this is with the %w verb in fmt.Errorf. Wrapped errors create a
		// logical chain (A wraps B, which wraps C, etc.) that can be queried with
