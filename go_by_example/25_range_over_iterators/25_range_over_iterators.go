...
* iterators, which lets us range over pretty much anything!
**/
package main
...
func (lst *List[T]) All() iter.Seq[T] {
	return func(yield func(T) bool) {
		...
		// element we want to iterate over, and note yield’s return value for a
...
// to be finite! Here’s a function returning an iterator over Fibonacci
// numbers: it keeps running as long as yield keeps returning true.
...
func main() {
	...
	for n := range genFib() {
		//  Once the loop hits break or an early return, the yield function passed to the
